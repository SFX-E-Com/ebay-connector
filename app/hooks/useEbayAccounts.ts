...
interface UseEbayAccountsReturn {
  ...
  createAccount: (data: { friendlyName: string; tags: string[]; ebayUsername?: string; }) => Promise<void>;
}
...
export function useEbayAccounts(): UseEbayAccountsReturn {
  ...
  const createAccount = useCallback(async (data: { friendlyName: string; tags: string[]; ebayUsername?: string; }) => {
    try {
      setLoading(true);
      const response = await fetch('/api/ebay-accounts', {
        method: 'POST',
